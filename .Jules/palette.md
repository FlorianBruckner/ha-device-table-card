...
## 2025-10-26 - [Accessible Accordions and Informative Disabled States]
**Learning:** To build truly accessible custom card configuration editors, interactive toggle headers must utilize the `aria-controls` attribute pointing directly to the unique ID of their expanded content panels. This allows screen readers to provide immediate structural navigation context. Additionally, hiding decorative SVGs inside buttons/headers via `aria-hidden="true"` prevents screen reader clutter, and dynamically updating `title`/`aria-label` on disabled buttons (such as Move Up/Down actions at the boundaries of lists) clarifies exactly why the actions are blocked.
**Action:** Always link accordion toggle headers to panels using `aria-controls`, set `aria-hidden="true"` on decorative inline SVGs, and provide descriptive explanations in `title`/`aria-label` for disabled button states.
