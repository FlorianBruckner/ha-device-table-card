...
## 2026-08-11 - [Empty Table State Interactive Clear Search Affordance]
**Learning:** When a search filter returns zero matches, showing a static "No matching records" message can lead to dead-ends. Providing an inline, interactive, and focusable "Clear search" link (with role="button" and tabindex="0") in the empty table state lets keyboard and screen-reader users seamlessly reset their filter state with a single press of Enter/Space or click, while automatically refocusing the search input for immediate query retry.
**Action:** Always include an interactive, focusable, and keyboard-accessible reset affordance inside zero-records empty placeholder messages, and handle focus recovery cleanly.
