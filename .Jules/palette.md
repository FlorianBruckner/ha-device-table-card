...
## 2026-08-12 - [Clear Search Link in Empty Table States]
**Learning:** When a user's search query returns no matching records, an empty table state can feel like a dead end. Providing a prominent, keyboard-focusable "Clear search" link directly inside the empty state allows users to easily clear their filter and instantly recover the full list of records with a single click or keyboard action. Ensuring the link is keyboard-navigable (`tabindex="0"`, `role="button"`) and supports focus states (`:focus-visible`) keeps the interaction accessible and highly satisfying.
**Action:** Always pair empty states from filtered queries with a prominent, accessible "Clear search" link that resets the active filter, redraws the content, and restores focus back to the search input.
