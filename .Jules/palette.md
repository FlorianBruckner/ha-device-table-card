...
## 2026-08-04 - [Dynamic Accessible Table Sorting Indicators]
**Learning:** Relying purely on CSS color highlights or native icons to convey table column sorting is insufficient for screen reader users and can be hard to notice on different monitor types. Utilizing DataTables' `drawCallback` hook to dynamically append sorting state (e.g., "sorted ascending, click to sort descending") to headers' `title` and `aria-label` provides a highly localized, reactive, and clear accessibility experience.
**Action:** When working with dynamic tables, leverage redraw events to keep the ARIA attributes of sorting headers perfectly synchronized with active states and potential keyboard click directions.
