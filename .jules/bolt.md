...
## 2026-07-29 - [Entity Loop Multi-Criteria Early Exit]
**Learning:** In `processDevices` within `src/data-processor.ts`, iterating through all raw device entities to find matches and tracking the latest updated state (for last_changed metric) is a common hot-path overhead. By introducing tracking for unique required class matches (`matchedClassesCount`) and checking it alongside `matchedSuffixesCount` and the anchor filter (`hasAnchor`), we can execute a multi-criteria early exit when `!needsLastChanged` is true. This skips unnecessary remaining entity state lookups, yielding a ~1.85x speedup for the majority of devices that have stable entities and do not render a dynamic last_changed column.
**Action:** Always look for multi-criteria early-exit conditions in entity matching loops when dynamic time or metric tracking is not requested.
