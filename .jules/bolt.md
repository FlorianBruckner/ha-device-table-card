## 2026-08-01 - [Fast-Path HTML Escaping for Hot Cell Rendering Paths]
**Learning:** HTML escaping in hot-path column cell renderers can become a major bottleneck during table redraws. Standard string escaping libraries perform extensive regex testing and string allocation even on totally safe strings and numeric values. By implementing a custom `fastEscape` helper that directly casts numbers and booleans (preserving `0` and `false` values) and uses a fast regex check (`/[&<>"']/`) to bypass the full escaping library call for safe values, we completely avoid expensive regular expression replacements and redundant allocations on the hot path of DataTable cell, header, and tooltip rendering passes.
**Action:** Implement a fast-path HTML escaping check using a quick character class test on safe values to skip library/regex overhead in cell renderers.

## 2026-07-31 - [Connection-Keyed Registry Cache & Test Isolation]
**Learning:** Multiple custom card instances rendering on the same dashboard all call `_fetchRegistries` concurrently during initial load. We can coalesce overlapping API requests and share constructed maps by caching the active `Promise` and parsed `SharedRegistryData` in a module-level `WeakMap` keyed on the HA WebSocket connection object (`this.hass.connection`). However, to maintain robust test isolation when mock connection objects are reused across tests with different callWS responses, the cache must validate that the `this.hass.callWS` helper reference has not changed, invalidating the entry on any reference drift.
**Action:** Always key dashboard-wide shared registry data caches on the WebSocket connection and validate the fetch helper's reference equality to guarantee correct isolated test environments.
...
## 2026-08-02 - [DataTable drawCallback Header Query Caching]
**Learning:** High-frequency table redraw events (occurring on user typing, pagination, filter, and sorting actions) call the `drawCallback` hook repeatedly. Within this hook, querying the DOM with `.querySelectorAll('table.dataTable thead th')` can become a major rendering and UI blocking bottleneck. Storing a private `_cachedHeaders` list on the class instance, and lazily setting it on the first draw, completely bypasses redundant DOM sweeps. Resetting the cached elements to `null` on table initialization and reinitialization guarantees correct DOM tracking across table configurations.
**Action:** Cache DOM query lists on class properties inside high-frequency lifecycle callbacks (like `drawCallback`) and invalidate/reset them during initializations.
