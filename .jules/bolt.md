...
## 2026-08-02 - [Lazy Metadata Extraction on Memoized Cache Verification]
**Learning:** When checking object cache validity in high-frequency loops (like `processDevices` in `src/data-processor.ts`), pre-extracting metadata properties (`d.name_by_user`, `d.name`, `d.area_id`, `d.manufacturer`) before evaluating the cache condition creates redundant property lookups and type checks on every hot hit. By deferring property extractions until after the object reference check (`d === cached.deviceRef`) fails or cache misses occur, short-circuit evaluation completely bypasses property extractions on hot memoized passes.
**Action:** Always place fast reference equality checks (`d === cached.ref`) at the beginning of cache verification conditions to short-circuit property extractions.
