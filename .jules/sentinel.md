...
## 2026-07-13 - [Hardened Array Type Checks for DoS and UI Crash Prevention]
**Vulnerability:** Input fields, configuration properties (such as `columns` and `highlight`), or backend API payload lists (like registries received over WebSocket) that are expected to be arrays could be manipulated or sent as primitive values or arbitrary objects. Attempting to traverse, spread, or mutate these structures directly without explicit type-checking causes unhandled `TypeError` exceptions and freezes/crashes the user interface, presenting a client-side Denial-of-Service vector.
**Learning:** Checking types explicitly using `Array.isArray()` before executing any mapping, spreading, splice, or copy operations on external or configuration-driven structural arrays ensures that the card and editor fail securely and fallback gracefully to default values.
**Prevention:** Never assume incoming configurations or API payloads conform strictly to typescript interface/type definitions at runtime. Always perform defensive `Array.isArray` type-checking at boundaries before utilizing arrays.
