...
## 2026-07-19 - [Registry Subscription Leak and accumulation on Connection Updates]
**Vulnerability:** Multiple subscription listeners were accumulating on the Home Assistant WebSocket connection if the connection object was renewed or if `hass` updated frequently, leading to severe client-side performance degradation and potential memory exhaustion/Denial of Service (DoS).
**Learning:** In Home Assistant custom cards, the `hass` reference is updated on every state change, but the `hass.connection` object remains stable unless a reconnection occurs. Tracking the active connection in `_subscribedConnection` and cleanly unsubscribing and clearing local subscription references on connection change is critical to preventing listener accumulation. Additionally, registry fetching must remain robust to state updates (always called but internally guarded) while event subscriptions are bound strictly to connection updates.
**Prevention:** Explicitly track and bind WebSocket event subscriptions to the connection lifecycle rather than arbitrary property/state updates, and ensure that all subscription reference arrays are completely cleared before registering new handlers.
