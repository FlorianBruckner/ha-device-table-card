...
## 2026-07-16 - [Harden State Attributes Safeness to Prevent DoS and Crashes]
**Vulnerability:** Unhandled property lookups on `stateObj.attributes` (such as `friendly_name` or `unit_of_measurement`) without optional chaining or safety checks could cause client-side TypeError crashes (localized Denial of Service) if entity state payloads have null or missing attributes.
**Learning:** Home Assistant state structures are highly dynamic and can occasionally contain null or missing attributes. Directly dereferencing nested properties on potentially missing objects in the rendering hot path breaks the entire UI. Optional chaining (`?.`) provides complete resilience against such TypeErrors.
**Prevention:** Always use optional chaining or explicit null/undefined checks when accessing nested attributes of objects that represent external registry or state payloads.
