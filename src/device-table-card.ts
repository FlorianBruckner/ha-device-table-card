...
export class DeviceTableCard extends LitElement implements LovelaceCard {
  ...
  private _sanitizeConfig<T>(obj: T, visited = new WeakSet<any>()): T {
    ...
    if (Array.isArray(obj)) {
      return obj.map((item) => this._sanitizeConfig(item, visited)) as any;
    }
    const sanitized: any = {};
    for (const key of Object.keys(obj)) {
      ...
      sanitized[key] = this._sanitizeConfig((obj as any)[key], visited);
    }
    return sanitized;
  }
  ...
  private _sanitizeColor(color: string): string {
    ...
    // Enforce standard input length limit (100 characters max) to protect from ReDoS (Denial of Service)
